...
class StationController extends Controller
{
     ...
     *
     * @Route("/new", name="crud_station_new")
     * @Method({"GET", "POST"})
     */
    public function newAction(Request $request)
    {
        $station = new Station();
        $form = $this->createForm('FAb\SensorsBundle\Form\StationType', $station);
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            return $this->redirectToRoute('crud_station_show', array('id' => $station->getId()));
        }
     ...
     *
     * @Route("/{id}", name="crud_station_show")
     * @Method("GET")
     */
     ...
     *
     * @Route("/{id}/edit", name="crud_station_edit")
     * @Method({"GET", "POST"})
     */
    public function editAction(Request $request, Station $station)
    {
        $deleteForm = $this->createDeleteForm($station);
        $editForm = $this->createForm('FAb\SensorsBundle\Form\StationType', $station);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $this->getDoctrine()->getManager()->flush();

            return $this->redirectToRoute('crud_station_edit', array('id' => $station->getId()));
        }
     ...
     *
     * @Route("/{id}", name="crud_station_delete")
     * @Method("DELETE")
     */
    public function deleteAction(Request $request, Station $station)
    {
        ...
        return $this->redirectToRoute('crud_station_index');
    }
    ...
    private function createDeleteForm(Station $station)
    {
        return $this->createFormBuilder()
            ->setAction($this->generateUrl('crud_station_delete', array('id' => $station->getId())))
            ->setMethod('DELETE')
            ->getForm()
        ;
    }
